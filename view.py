...
class CalibracaoHX711:
    ...
    def criar_interface(self):
        ...
        ttk.Label(frame_serial, text="Porta:").grid(row=0, column=0, padx=5,pady=5)
        ...
        ttk.Label(frame_tara, text="Tara:").grid(row=0, column=1)
        
        ttk.Label(frame_tara, textvariable=self.tara_var).grid(row=0, column=2, padx=10)
        ...
        ttk.Entry(frame_ponto, textvariable=self.peso_var, width=15).grid(row=0, column=1, padx=5)
        
        ttk.Button(frame_ponto, text="ADICIONAR PONTO", command=self.adicionar_ponto).grid(row=0, column=2, padx=10)
        ...
        self.tabela.heading("bruto", text="Bruto")
        ...
        ttk.Button(frame_calibracao, text="FINALIZAR CALIBRAÇÃO", command=self.finalizar_calibracao).grid(row=0,column=0, padx=10, pady=10)
    ...
    def adicionar_ponto(self):
        ...
        self.tabela.insert( "", tk.END, values=(numero, peso, bruto, corrigido))
        self.peso_var.set("")
    ...
    def finalizar_calibracao(self):
        ...
        self.B = float(coeficientes[1])
        self.C = float(coeficientes[2])

        # Valores calculados pela curva
        y_calculado = (self.A * x**2 + self.B * x + self.C)

        # Cálculo do R²
        ss_res = np.sum((y - y_calculado) ** 2)
        ...
        self.resultado.insert(tk.END, "CURVA DE CALIBRAÇÃO\n\n")
        self.resultado.insert(tk.END, "Peso(g) = A*x² + B*x + C\n\n")
        self.resultado.insert(tk.END, f"A = {self.A:.15e}\n")
        self.resultado.insert(tk.END, f"B = {self.B:.15e}\n")
        self.resultado.insert(tk.END, f"C = {self.C:.15e}\n")
        self.resultado.insert(tk.END, f"R² = {r2:.10f}\n\n")
        self.resultado.insert(tk.END, "x = leitura bruta - tara\n")
        self.resultado.insert(tk.END, f"tara = {self.tara}\n")
        messagebox.showinfo("Calibração", "Curva calculada com sucesso.")

    # Enviar curva
    def enviar_curva(self):
        if len(self.pontos) < 3:
            messagebox.showerror("Erro", "Calcule a curva primeiro")    
            return
        
        comando = (f"COEF," f"{self.A:.15e}," f"{self.B:.15e}," f"{self.C:.15e}")
        resposta = self.enviar_comando(comando)
        if "COEF_OK" in  resposta:
            messagebox.showinfo("Curva", "Coeficientes eviados para o ESP32.")
        else:
            messagebox.showwarning("Curva", "\n".join(resposta))
            
    # Salvar
    def salvar_calibracao(self):
        if len(self.pontos) < 3:
            messagebox.showerror("Erro", "Não há uma calibração válida.")
            return
        
        arquivo = filedialog.asksaveasfilename(title="Salvar calibração", defaultextension=".json",
                                           filetypes=[("Arquivo JSON","*.json")])

        if not arquivo:
            return
        
        dados = {"tara": self.tara, "numero_leituras": int(self.leituras_var.get()),
                    "coeficientes": {"A":self.A, "B":self.B, "C":self.C}, "pontos": self.pontos}
        
        try:
            with open(arquivo, "w", encoding="utf-8") as f:
                json.dump(dados, f, indent=4, ensure_ascii=False)

            messagebox.showinfo("Salvar", "Calibração salva com sucesso.")
        except Exception as erro:
